...
//////////////////////////////////////////
  ...
  tt{
    ...
    while(k--){
      for(int i=0;i<n-1;i++){
        if(a[i]<a[i+1]){ // delete current element and shift all
          for(int j=i;j<n-1;j++)a[i]=a[i+1];
        }
        flag++; 
      }
    }
    //delete last element if there is no deleted element
    if(flag==0)a[n-1]=-1;
      for(itn i=0;i<n;i++){
        if(a[i] !=-1 && a[i] !=a[i+1])cout<<a[i]<<" ";
      }
    cout<<endl;
  }
  return 0;
